...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        if (data == null) {
            throw new IllegalArgumentException(
                    "Tried to add null data to ArrayDeque");
        }

        if (size == backingArray.length) {
            // resize on size equal to capacity
            T[] newArr = (T[]) new Object[size * 2];
            for (int i = 0; i < size; i++) {
                newArr[i + 1] = backingArray[mod(front + i,
                        backingArray.length)];
            }
            front = 0;
            backingArray = newArr;
    ...
    public void addLast(T data) {
        ...
        if (size == backingArray.length) {
            ...
            T[] newArr = (T[]) new Object[size * 2];
            for (int i = 0; i < size; i++) {
                newArr[i] = backingArray[mod(front + i, backingArray.length)];
            }
            backingArray = newArr;
            front = 0;
     ...
     *
     * If the deque becomes empty as a result of this call, do not reset front
     * to 0. Rather, modify the front index as if the deque did not become empty
    ...
    public T removeFirst() {
        if (size == 0) {
            throw new NoSuchElementException(
                    "Tried to remove value from empty ArrayDeque");
        }
    ...
    public T removeLast() {
        ...
        T removed = backingArray[mod(front + size - 1, backingArray.length)];
    ...
    public T getFirst() {
        if (size == 0) {
            throw new NoSuchElementException(
                    "Tried to get value from empty ArrayDeque");
        }
     ...
     *
     * This differs from using the % operator in that the % operator returns the
     * smallest answer with the same sign as the dividend. So, for example, (-5)
     * % 6 => -5, but with this method, mod(-5, 6) = 1.
     *
     * Examples: mod(-3, 5) => 2 mod(11, 6) => 5 mod(-7, 7) => 0
     *
     * This helper method is here to make the math part of the circular behavior
     * easier to work with.
     *
